...
test.describe('API JSON files', () => {
  ...
  test('websites.json and platforms.json have correct content-type', async ({ request }) => {
    const websitesResponse = await request.get('/api/websites.json')
    const platformsResponse = await request.get('/api/platforms.json')

    const websitesContentType = websitesResponse.headers()['content-type']
    const platformsContentType = platformsResponse.headers()['content-type']

    // Content-Type should include 'application/json'
    expect(websitesContentType).toContain('application/json')
    expect(platformsContentType).toContain('application/json')
  })
})
